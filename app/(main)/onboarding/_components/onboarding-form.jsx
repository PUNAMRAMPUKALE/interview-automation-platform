...
// ⛔️ REMOVE: import { updateUser } from "@/actions/user";
// ✅ We’ll receive it from the server page:
const OnboardingForm = ({ industries, updateUserAction }) => {
  ...
  const {
    ...
    fn: updateUserFn,
    data: updateResult,
  } = useFetch(updateUserAction); // ⬅️ use the server action passed down
        ...
        <CardContent>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <Button type="submit" className="w-full" disabled={updateLoading}>
              {updateLoading ? (<><Loader2 className="mr-2 h-4 w-4 animate-spin" />Saving...</>) : ("Complete Profile")}
            </Button>
          </form>
        </CardContent>
      </Card>
